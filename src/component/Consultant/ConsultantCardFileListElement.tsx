...
import { isoUTCOffsetToNZDateTimeObject } from "../../utility/date/dateUtility";
...
function ConsultantCardFileListElement(props: Props) {
  ...
  return (
    <li className="flex flex-1 w-full flex-col p-6 shadow-md rounded-xl hover:bg-gray-100 hover:-translate-y-3 cursor-pointer transition-all duration-100 ease-in-out group border border-black/15 space-y-1">
      <Link to="/project/$projectId" params={{ projectId: file.id }}>
        <div className="flex w-full justify-between 0 pb-3">
          <p className="text-lg">{fileNameObject.surname}</p>
          <p className="text-xs text-gray-500 italic">
            {isoUTCOffsetToNZDateTimeObject(file.lastModified)}
          </p>
        </div>
