...
export function getLewissPhoenixwoodFabricCost(
  width: number,
  height: number,
  slatSize: number,
...
function getControlMultiplier(
  ...
  const controlSlatType = `phoenixwood${slatSize}`;
  ...
  const controlObject =
    controlSlatTypeObject[
      controlKey as keyof typeof pricingSchedule.control.phoenixwood50
    ];
